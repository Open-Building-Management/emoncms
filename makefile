...
module:
	@if [ ! -d "$(WWW)/emoncms/Modules/$(name)" ]; then\
		...
		cd $(WWW)/emoncms/Modules && git clone -b $(addon_branch) https://github.com/emoncms/$(name);\
	fi
...
symodule:
	...
	@if [ ! -d "$(EMONCMS_DIR)/modules/$(name)" ]; then\
		echo "Installing module $(name)";\
		cd $(EMONCMS_DIR)/modules && git clone -b $(addon_branch) $(SYMLINKED_MODULES_URL)/$(name);\
	fi
